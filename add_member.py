...
async def client_message(client_ms):
    channel = await client_ms.get_entity('Fx Phonix')
...
def add_member(input_config, output_config, root_path_config, start_time_config, folder_session_config):
    ...
    for account in accounts:
        ...
        client = TelegramClient(folder_session_config + phone, api_id, api_hash)
    ...
    for my_client in clients:
        ...
        if phone != "+84585771080":
            path_group = root_path_config + '/data/group/' + phone + '.json'
            if os.path.isfile(path_group):
                ...
                if current_target_group:
                    ...
                    path_group_user = root_path_config + '/data/user/' + phone + "_" + str(group_source_id) + '.json'
    ...
    try:
        with open(root_path_config + '/' + output_config + '.txt') as f:
    ...
    while i < total_user:
        ...
        try:
            ...
            loop.run_until_complete(client_message(is_client_message))
    ...
    print("total time: " + str(end_time - start_time_config))
